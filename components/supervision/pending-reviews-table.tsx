...
import { Eye, XCircle, Calendar, User, BookOpen } from "lucide-react"
...
export function PendingReviewsTable() {
  ...
  const [isApprovingFromModal, setIsApprovingFromModal] = useState(false)
  ...
  const handleApproveFromModal = async () => {
    if (!selectedDetailData) return

    setIsApprovingFromModal(true)
    try {
      // Simular llamada a API
      await new Promise((resolve) => setTimeout(resolve, 1000))
      console.log("Aprobando secuencia desde modal:", selectedDetailData.id)

      // Cerrar modal después de aprobar
      setIsDetailModalOpen(false)
      setSelectedDetailData(null)
    } catch (error) {
      console.error("Error al aprobar:", error)
    } finally {
      setIsApprovingFromModal(false)
    }
  }

  const handleRejectFromModal = () => {
    // Cerrar modal de detalle y abrir modal de rechazo
    setIsDetailModalOpen(false)
    if (selectedDetailData) {
      const originalItem = pendingReviewsData.find((item) => item.id === selectedDetailData.id)
      if (originalItem) {
    ...
    <Card>
      ...
      <CardContent>
        ...
        <SequenceDetailModal
          ...
          isApproving={isApprovingFromModal}
          isRejecting={isRejectingFromModal}
        />
      </CardContent>
    </Card>
  )
